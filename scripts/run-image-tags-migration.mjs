#!/usr/bin/env node
/**
 * Runs the image_tags table migration against Supabase.
 * Usage: npm run db:migrate-image-tags
 */
...
const sqlPath = join(__dirname, 'run-image-tags-migration.sql')
...
async function run() {
  ...
  for (let i = 0; i < connectionStrings.length; i++) {
    ...
    try {
      ...
      await runWith(url)
      console.log('image_tags migration completed.')
      return
    } catch (err) {
      lastErr = err
